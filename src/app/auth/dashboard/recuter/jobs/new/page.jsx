...
import { creatJob } from "@/lib/action/job";
...
export default function PostJobPage() {
    ...
    const handleSubmit = async (e) => {
        ...
        const res = await creatJob(payload);
        console.log(" API Response:", res);
        if (res.insertedId) {
            console.log(" Job posted successfully! ID:", res.insertedId);
            toast.success("Job posted successfully!");
            e.target.reset();
            setIsRemote(false);
            redirect("/auth/dashboard/recuter/jobs");
        } else {
            console.error(" Error posting job:", res);
        }
    };
                ...
                <Form onSubmit={handleSubmit} className="space-y-8" validationErrors={errors} validationBehavior='aria'>
                    ...
                    <Fieldset className="space-y-6 w-full">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <Select id="jobCategory" className={selectBoxClass} name="jobCategory" isInvalid={!!errors.jobCategory}>
                                <Label htmlFor="jobCategory" className="text-zinc-400 font-medium text-sm mb-1 block">Job Category</Label>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <Select id="jobType" className={selectBoxClass} name="jobType" isInvalid={!!errors.jobType}>
                                <Label htmlFor="jobType" className="text-zinc-400 font-medium text-sm mb-1 block">Job Type</Label>
                                <Select.Trigger className={triggerClasses}>
                                    <Select.Value />
                                    <Select.Indicator />
                                </Select.Trigger>
                                ...
                                <div className="col-span-2 space-y-1">
                                    ...
                                    <div className="flex gap-2">
                                        <TextField name="minSalary" isInvalid={!!errors.minSalary} className="w-full">
                                            ...
                                            {errors.minSalary && <FieldError className="text-xs text-danger mt-1">{errors.minSalary}</FieldError>}
                                        </TextField>
                                        <TextField name="maxSalary" isInvalid={!!errors.maxSalary} className="w-full">
                                            ...
                                            {errors.maxSalary && <FieldError className="text-xs text-danger mt-1">{errors.maxSalary}</FieldError>}
                                        </TextField>
                                    </div>
                                </div>

                                <Select id="currency" className="w-full mt-6" name="currency" defaultSelectedKeys={["USD"]}>
